    ...
    <div class="main-content" style="margin-left:280px;padding:2rem;">
        <div class="container-fluid px-0">
            <div class="page-header">
                <h1 class="page-title"><i class="fas fa-edit me-2"></i>Edit Foto</h1>
                <p class="page-subtitle">Ubah informasi dan gambar foto</p>
            </div>
            ...
            <div class="main-content">
                ...
                <form action="{{ route('admin.photos.update', $photo->id) }}" method="POST" enctype="multipart/form-data" id="photoForm">
                    @csrf
                    @method('PUT')
                    
                    <div class="form-group">
                        <label for="judul" class="form-label"><i class="fas fa-heading me-2"></i>Judul Foto</label>
                        <input type="text" class="form-control" id="judul" name="judul" value="{{ old('judul', $photo->judul) }}" placeholder="Masukkan judul foto" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="deskripsi" class="form-label"><i class="fas fa-align-left me-2"></i>Deskripsi</label>
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="4" placeholder="Masukkan deskripsi foto (opsional)">{{ old('deskripsi', $photo->deskripsi) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="kategori_id" name="kategori_id" required>
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('kategori_id', $photo->kategori_id) == $category->id ? 'selected' : '' }}>{{ $category->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label"><i class="fas fa-image me-2"></i>Foto Saat Ini</label>
                        <div class="text-center mb-3">
                            <img src="{{ asset('storage/' . $photo->file_path) }}" alt="{{ $photo->judul }}" class="current-image" id="currentImage">
                        </div>
                        <small class="text-muted">Biarkan kosong jika tidak ingin mengubah foto</small>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label"><i class="fas fa-image me-2"></i>Upload Foto Baru (Opsional)</label>
                        <div class="file-upload-area" id="fileUploadArea">
                            <div class="file-upload-icon"><i class="fas fa-cloud-upload-alt"></i></div>
                            <div class="file-upload-text">Klik atau Drag & Drop untuk Upload Foto Baru</div>
                    ...
                    <button type="submit" class="btn-submit"><i class="fas fa-save me-2"></i>Update Foto</button>
                </form>
